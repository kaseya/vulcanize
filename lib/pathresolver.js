...
var PathResolver = function PathResolver(abspath, cssUrlPrefix) {
  if (abspath) {
    this.abspath = abspath;
  }
  if (cssUrlPrefix) {
    this.cssUrlPrefix = cssUrlPrefix;
  }
};
...
rewriteURL: function rewriteURL(importUrl, mainDocUrl, cssText) {
    return cssText.replace(constants.URL, function(match) {
      ...
      // remove < .. > before adding < /liveconnect > in front of the url.  
      if (this.cssUrlPrefix && path.indexOf(this.cssUrlPrefix) !== 0 && path.slice(0,6) == '../lib') {
        path = path.substring(2)
        return 'url("' + this.cssUrlPrefix + path + '")';
      } else {
        return 'url("' + path + '")';
      }
    }.bind(this));
